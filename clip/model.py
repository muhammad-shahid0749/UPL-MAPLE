...
class ResidualAttentionBlock_UPLTrainer(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['UPLTrainer_length']
        if i == 0:
    ...
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_UPLTrainer(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer_UPLTrainer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details,shared_ctx, compound_deeper_prompts):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "UPLTrainer":
                self.visual = VisionTransformer_UPLTrainer(
                    input_resolution=image_resolution,
