...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        #assert cfg.TRAINER.UPLTrainer.PROMPT_DEPTH >= 1, "For MaPLe, PROMPT_DEPTH should be >= 1"
        #self.compound_prompts_depth = cfg.TRAINER.UPLTrainer.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
...
class UPLTrainer(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                output, image_features, text_features = self.model(image,label)
